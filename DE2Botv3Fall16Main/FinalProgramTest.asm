...
Main:
	...
	LOAD   Mask2
	OR     Mask3       ; AC now contains 0b01100
	OUT    SONAREN     ; enable front-facing sensors
	
SimpleTask:
	; check for objects in front of robot
	IN     DIST2       ; get value from sensor 2
	ADDI   -610        ; about 2ft
	; Because the new SCOMP has an 11-bit operand, we can use
	; immediate values up to +/-1023
	JNEG   ReturnHome5         ; routine to immediately stop the robot
	...
	; robot should be moving in +X direction.
	IN     XPOS
	SUB    TwoFeet     ; just using because it was already defined
	SUB    TwoFeet
	JPOS   ReturnHome5
	
	; If no case above stops the robot, then at this point
	; the robot should continue moving forward.
	LOAD   FFast        ; defined below
...
TurnH:
	...
	ADDI -3
	MOVR r10, r0	; load r10 with desired angle -2 deg
	ADDI 6
	MOVR r11, r0	; load r11 with angle +2 deg
	CMP r9, r10
	JPOS CheckH		; theta is larger than low range of desired angle
	JUMP TurnH	
CheckH:
	CMP r9, r11		
	JNEG Rev		; theta is smaller than high range of desired angle
	JUMP TurnH
...
Rev2:				; X-coordinates are good, need to check Y
					; Actually turn until it faces 90 degrees
	LOAD ZERO
	OUT RVelCmd
	IN THETA
	SUB Deg90
...
;STOPHOME:
	;LOAD ZERO ; TEST CODE
	;OUT LVELCMD ; TEST CODE
	;LOAD ZERO ; TEST CODE
	;OUT RVELCMD ;TEST CODE
	;JUMP STOPHOME ;TEST CODE
;	JPOS RevCW
;	JNEG RevCW
